				...
				<form name="frm" method="post" action="/board/selectList.do">
					<fieldset>
						...
						<select name="searchCondition" id="ftext">
							<option value="0" <c:if test="${searchVO.searchCondition eq '0'}">selected="selected"</c:if>>
								제목
							</option>
							<option value="1" <c:if test="${searchVO.searchCondition eq '1'}">selected="selected"</c:if>>
								내용
							</option>
							<option value="2" <c:if test="${searchVO.searchCondition eq '2'}">selected="selected"</c:if>>
								작성자
							</option>
						</select>
						...
						<tbody>
							...
							<c:forEach var="result" items="${resultList}" varStatus="status">
								<tr>
									...
									<td class="hit" data-cell-header="조회수: "><c:out value="${result.inqireCo}"/></td>
								</tr>
							</c:forEach>
